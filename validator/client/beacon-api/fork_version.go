...
func (c beaconApiValidatorClient) getForkVersion(epoch types.Epoch) ([4]byte, error) {
	...
	if err != nil {
		return forkVersion, errors.Wrap(err, "failed to query REST API genesis endpoint")
	}
	defer func() {
	...
	if err != nil {
		return forkVersion, errors.Wrap(err, "failed to decode genesis fork version")
	}
